---
title: "Fetching Odds"
description: "Learn how to fetch and compare odds from multiple bookmakers"
---

## Overview

The Odds-API.io provides several endpoints for fetching odds data from 250+ bookmakers. This guide covers best practices and common patterns for working with odds data.
...
- **Asian Handicap** - Handicap betting with fractional lines
- **Over/Under** - Total goals/points over or under a line
...
function findBestOdds(oddsData) {
  ...
  for (const [bookmaker, markets] of Object.entries(oddsData.bookmakers)) {
    const market1x2 = markets.find(m => m.name === market);
    if (!market1x2?.odds[0]) continue;

    const odds = market1x2.odds[0];
...
Each odds value includes a direct link to the bookmaker's bet slip:

```javascript
const odds = oddsData.bookmakers.Bet365[0].odds[0];

// Direct link to place a bet on the home team
const betLink = odds.homeLink;

// Use this in your UI
<a href={betLink} target="_blank">
  Bet on {oddsData.home} @ {odds.home}
</a>
```
...
<AccordionGroup>
  <Accordion title="Select Only Needed Bookmakers">
    Don't fetch odds from all 250+ bookmakers. Select the most relevant bookmakers for your users (max 30 per request).
  </Accordion>
