...
description: "Access real-time player prop odds from 250+ bookmakers including DraftKings and FanDuel. Get points, assists, passing yards for NBA, NFL, MLB, NHL."
---
...
```bash cURL
curl "https://api2.odds-api.io/v3/odds?apiKey=YOUR_API_KEY&eventId=123456&bookmakers=DraftKings,FanDuel"
```
...
class PropsMonitor {
  ...
  async checkForLineMovement() {
    const response = await fetch(
      `https://api2.odds-api.io/v3/odds?apiKey=${this.apiKey}&eventId=${this.eventId}&bookmakers=${this.bookmakers}`
    );
...
async function combineFantasyAndProps(eventId, fantasyProjections) {
  ...
  const response = await fetch(
    `https://api2.odds-api.io/v3/odds?apiKey=${apiKey}&eventId=${eventId}&bookmakers=DraftKings`
  );
