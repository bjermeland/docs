...
description: "Build a betting odds comparison tool with Odds-API.io. Compare prices from 250+ bookmakers in real-time and find the best odds for any event."
---
...
const response = await fetch(
  `https://api2.odds-api.io/v3/events?apiKey=${apiKey}&sport=football&league=england-premier-league&status=pending&limit=10`
);
...
const bookmakers = 'Bet365,SingBet,Unibet,William Hill,Betway';
...
Home (Manchester United): 2.15 at SingBet
Draw: 3.45 at Unibet
...
--------------------------------------------------------------------------------
SingBet            2.15           3.35           3.15           10:29:50 AM
...
async function compareLeagueOdds(sport, league) {
  ...
  const eventsRes = await fetch(
    `https://api2.odds-api.io/v3/events?apiKey=${apiKey}&sport=${sport}&league=${league}&status=pending&limit=20`
  );
  ...
  const oddsRes = await fetch(
    `https://api2.odds-api.io/v3/odds/multi?apiKey=${apiKey}&eventIds=${eventIds}&bookmakers=${bookmakers}`
  );
...
app.get('/api/compare/:eventId', async (req, res) => {
  ...
  const bookmakers = 'Bet365,SingBet,Unibet,William Hill,Betway,DraftKings,FanDuel';
